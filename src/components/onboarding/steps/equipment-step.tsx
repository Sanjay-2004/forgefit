...
export function EquipmentStep() {
  ...
  const toggleEquipment = (equipment: EquipmentAccess) => {
    const selected = data.equipment_access || [];
    if (selected.includes(equipment)) {
      updateData({ equipment_access: selected.filter(e => e !== equipment) });
    } else {
      updateData({ equipment_access: [...selected, equipment] });
    }
  };
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-text-secondary">
          Select all that apply. We&apos;ll design your program around it.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {EQUIPMENT_OPTIONS.map((option, index) => {
          const isSelected = (data.equipment_access || []).includes(option.value as EquipmentAccess);
          return (
            <motion.button
              key={option.value}
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => toggleEquipment(option.value as EquipmentAccess)}
      ...
      <Button
        onClick={nextStep}
        disabled={(data.equipment_access || []).length === 0}
        fullWidth
        size="lg"
      >
        Continue
